...
double residual(const double *restrict, const double *restrict,
                const struct Opts);
...
void jacobi(double *restrict u, const double *restrict rho,
            const struct Opts o)
{
    double *u0 = malloc(o.n * sizeof(double));
    size_t i, j;
    for (i = 0; i < o.its; ++i) {
    #pragma omp parallel for default(none) shared(u0,u,o) private(j)
        for (j = 0; j < o.n; ++j)
            u0[j] = u[j];
...
double residual(const double *restrict u, const double *restrict rho,
                const struct Opts o)
{
    double eqn, res = 0;
    size_t j;
...
void output(const double *restrict u, const double *restrict rho,
            const size_t n)
{
